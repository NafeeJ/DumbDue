...
import com.kiwicorp.dumbdue.ui.reminder.TimeGroup
import com.kiwicorp.dumbdue.ui.reminder.ReminderViewModel
...
EditReminderFragment.OnReminderEditListener {
    ...
    companion object {
        ...
        val overdueList = LinkedList<OldReminder>()
        val todayList = LinkedList<OldReminder>()
        val tomorrowList = LinkedList<OldReminder>()
        val next7daysList = LinkedList<OldReminder>()
        val futureList = LinkedList<OldReminder>()
        ...
        fun insertReminderInOrder(list: LinkedList<OldReminder>, reminder: OldReminder) {
        ...
        private fun findReminderSection(reminder: OldReminder): ReminderSection {
            return when {
        ...
        fun getCorrectList(remindCalendar: Calendar): LinkedList<OldReminder> {
            return when {
        ...
        fun saveAll(context: Context) {
            val remindersListArray: Array<LinkedList<OldReminder>> = arrayOf(overdueList, todayList,
                tomorrowList, next7daysList, futureList)
            val sharedPreferences = context.getSharedPreferences("Preferences", Context.MODE_PRIVATE)
            val myGson = Gson()
            val editor: SharedPreferences.Editor = sharedPreferences.edit()
            //put global request code as a json
            val requestCodeJson: String = myGson.toJson(OldReminder.globalRequestCode)
            editor.putString("GlobalRequestCode", requestCodeJson)
            val reminderList = LinkedList<OldReminder>()
...
//        }
        ...
        reminderViewModel.getAllReminders().observe(viewLifecycleOwner, Observer {
            //update recycler view
            Toast.makeText(context,"I AM UPDATING",Toast.LENGTH_SHORT).show()
        })
        reminderViewModel.getReminders(TimeGroup.OVERDUE).observe(viewLifecycleOwner, Observer {
            //update RecyclerView
            Toast.makeText(context,"OVERDUE UPDATING",Toast.LENGTH_SHORT).show()
//            overdueSection.setList(it)
//            sectionAdapter.notifyDataSetChanged()
        })
        reminderViewModel.getReminders(TimeGroup.TODAY).observe(viewLifecycleOwner, Observer {
            //update RecyclerView
            Toast.makeText(context,"TODAY UPDATING",Toast.LENGTH_SHORT).show()
//            todaySection.setList(it)
//            sectionAdapter.notifyDataSetChanged()
        })
        reminderViewModel.getReminders(TimeGroup.TOMORROW).observe(viewLifecycleOwner, Observer {
            //update RecyclerView
            Toast.makeText(context,"TOMORROW UPDATING",Toast.LENGTH_SHORT).show()
//            tomorrowSection.setList(it)
//            sectionAdapter.notifyDataSetChanged()
        })
        reminderViewModel.getReminders(TimeGroup.NEXT_7_DAYS).observe(viewLifecycleOwner, Observer {
            //update RecyclerView
            Toast.makeText(context,"NEXT 7 DAYS UPDATING",Toast.LENGTH_SHORT).show()
//            next7DaysSection.setList(it)
//            sectionAdapter.notifyDataSetChanged()
        })
        reminderViewModel.getReminders(TimeGroup.FUTURE).observe(viewLifecycleOwner, Observer {
            //update RecyclerView
            Toast.makeText(context,"FUTURE UPDATING",Toast.LENGTH_SHORT).show()
//            futureSection.setList(it)
//            sectionAdapter.notifyDataSetChanged()
        })
...
//        itemTouchHelper.attachToRecyclerView(recycler_view)
    ...
    private fun initializeSections() {
        ...
        overdueSection = ReminderSection("Overdue",this)
        todaySection = ReminderSection("Today",this)
        tomorrowSection = ReminderSection("Tomorrow",this)
        next7DaysSection = ReminderSection("Next 7 Days",this)
        futureSection = ReminderSection("Future",this)
...
//        val gson = Gson()
//        val emptyListJson = gson.toJson(LinkedList<OldReminder>())//an empty list in case lists haven't been stored
...
//        val reminderListType = object : TypeToken<LinkedList<OldReminder>>() {}.type
//        //get list
//        val reminderListFromJson = gson.fromJson<LinkedList<OldReminder>>(reminderListJson,reminderListType)
//        //load reminders
...
//        OldReminder.globalRequestCode = indexFromJson
//    }
...
//        val removedReminder: OldReminder = findReminderSection(viewHolder.adapterPosition).getList()[sectionAdapter.getPositionInSection(viewHolder.adapterPosition)]
//
...
//        if (removedReminder.repeatVal != OldReminder.REPEAT_NONE) {
//            val calendar = removedReminder.remindCalendar
//            //todo implement custom repeat
//            when(removedReminder.repeatVal) {
//                OldReminder.REPEAT_DAILY -> { calendar.add(Calendar.DAY_OF_YEAR, 1) }
//                OldReminder.REPEAT_WEEKDAYS -> {
...
//                    }
//                }
//                OldReminder.REPEAT_WEEKLY -> { calendar.add(Calendar.WEEK_OF_YEAR, 1) }
//                OldReminder.REPEAT_MONTHLY -> { calendar.add(Calendar.MONTH, 1) }
//                OldReminder.REPEAT_YEARLY -> { calendar.add(Calendar.YEAR,1) }
//            }
//            removedReminder.reAddReminder()
//        }
...
//                val updatedList: LinkedList<OldReminder> = updatedSection.getList()
...
//        args.putParcelable("ReminderData", (sectionAdapter.getSection(sectionTitle) as ReminderSection).getList()[sectionAdapter.getPositionInSection(itemPosition)].getReminderData())
        navController.navigate(R.id.action_main_reminder_to_edit_reminder,args)
    }
...
//        deleteReminder((sectionAdapter.getSection(oldSectionTitle) as ReminderSection).getList()[oldPositionInSection])
        OldReminder(newText,newRemindCalendar,newRepeatVal,newAutoSnoozeVal,context!!)
    }

    private fun deleteReminder(reminder: OldReminder) {
